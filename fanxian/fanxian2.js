...
function cashback() {
    ...
    function getData() {
        ...
        fxAjax.callBack = function ($data) {
            ...
            var type = '',html = '',hitime,timeCount=new contrast_time;
            dataLen = getD.length;
            //拼接html结构
            for(;i<dataLen;i++){
                type = getD[i].type;
                ...
                if(type == '7'){
                    html+='<li><img src=\"'+ getD[i].headimg+'\" onerror="imgErr(this)">';
                    html+='<p><font>'+ getD[i].nickname +'</font><span class="country"></span><span class="red right">返现'+ getD[i].money +'元</span></p>';
                    html+='<p><span class="desc">'+ '商城购物' +'</span><span class="right">'+ timeCount.fn(hitime,getD[i].update_time) +'</span></p></li>';
                }
            }
            dom.innerHTML += html;
        };
