...
//大奖初始化 清空圆的内容
	function prizeInit(data){
		...
		mainVar.M_mainBody.down(function(){
			...
			p.innerHTML=CONFIG.chars.kaijiang.a.replace("玩家","<br>玩家");
	...
	function getJJ(){
		...
		a.callBack=function($data){						
			var data=toObject($data),num=+data.data||1,parts=data.participants||1;
				num=data.balanceDayTipsSize||(mainVar.moneyNum*num).toFixed(0)
				parts=(mainVar.baseNum*parts).toFixed(0)
				num=num.toString()
				mainVar.peopleNum=parts;
				if(mainVar.jj)mainVar.jj.innerHTML="<b>"+num+"</b>";
				if(mainVar.people)mainVar.people.innerHTML="<b>"+parts+"</b>";
			};
		a.err=function(){};
		a.send()
	}	
	...
	function lingjiangs($,options){
		...
		t.init=function(obj,tag){
			...
			if(($rate==1||$isT==1||$typ==1)&&!mainVar.writeInfo&&activityType==1){
				isWriteInfo({
					isOpenAddress:$typ==1?true:false,
					fn:function(){
						var ele=mainVar.ele;
		...
		t.getHongbao=function(){
			location.replace(HOST.CJ+'/open/auth/ttdsb?orderId='+$orderId+'&openId='+mainVar.userInfo.openid)
			}
		t.getHongbaoDis=function(){
...
//边看边聊导航栏跳转链接
function goUrl($){var h='',user=mainVar.userInfo,me=0;
	switch($){
		case "a":case "M_WDSC":
			me="收藏夹";
			h="mywin.html"
	...
	setTimeout(function(){goto(h)},200);
}
		...
		t.opening=function(data){
			...
			//	countAction({option:'pageCount',method:'accumulate',dom:svgBox2,type:"SCENE",counterName:'主页访问量',instanceId:PAGE.countInstanceid});
					...
					if(mainData.redSource==="platform"){
						mainVar.moneyNum=1
						CONFIG.chars.kaijiang.a="大奖揭晓还有<label class='info_a'><b>0</b></label>秒，<b class='info_b'>1</b>位观众在一起摇，今日提现大锦囊<label class='info_c'><b>0</b></label>元！<label class='deng'>不要走开，摇起来，这次可能就是你哦！</label>"
						}
		...
		var host="//qa.h5.mtq.tvm.cn/yao/join_us/",user=mainVar.userInfo;
			setJsonp(host+"register.js?",function(){
				register({
					...
					openid:options.ttdsbOpenid,
					userInfoData:options.userInfoData,
...
function throughTrain(){
	var mtid=mainVar.mainData.time_interval_id,appName=mainVar.sceneData.appName,str='',column_id='',user=mainVar.userInfo;
	if(appName=='SHEQU'||appName=='ZHIBO'){
		var column_info=mainVar.mainData.column_info
		column_id=column_info?column_info.column_id:1;
	}else{
		str='&mtid='+mtid;
	};
	getTvme({
		callBack:function(){
			...
			mainVar.tvmE.getadbyca(column_id,mainVar.location.adcode,mainVar.location.region,function($data){
			...
			},encodeURIComponent("channelid="+PAGE.channelId+'&openid='+user.openid+'&nickname='+user.nickname+'&sex='+user.sex+str));
		}
	});
};
	...
	ajax.callBack=function($data){
		...
		if(data.status){
			if(data.data.info_status==2){
				if(options.isOpenAddress){
					writeInfo({
						mobile:data.data.mobile,
						ttdsbOpenid:data.data.openid,
						userInfoData:data,
						fn:options.fn
					});
				}else{
					...
					mainVar.personInfo={receiver:info.addressee,mobile:info.phone,addr:info.province+info.city+info.area+(info.town||'')+info.street};
					options.fn&&options.fn();
				};
			}else{
				writeInfo({
