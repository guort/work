...
.headImg{left:30px}\
	...
	(function(){
		var a=setAjax("GET",HOST.YAO+"/api/yaotv/index?action=yaocontrol&channelid="+PAGE.channelId)	
			a.callBack=function($data){
				var data=toObject($data)
				pageinit(user,nickname,data.menus)
			}
		a.send()		
	}());	
	tjData.page="我的金币";
	...
	function pageinit(user,nickname,menus){
		DO.title="个人中心";
		...
		,wdsc="OFF",wdsc_syt=""
		,sc="OFF",sc_sty=""
		,ylc="OFF",ylc_sty="border:none"
		,shangcheng,scdd
		,fx=+SWITCH.fanxianmingxi,fx_sty=""
		,tiwen=+SWITCH.tiwen,tiwen_sty=""
			for(var i=0,ie;i<xdM.length;i++){
				ie=xdM[i].txt
				if(ie==="M_WDSC")wdsc="M_WDSC"
				else if(ie==="M_SC_NEW")sc="M_SC_NEW"
				else if(ie==="SC")sc="M_SC"
				else if(ie==="M_YL")ylc="M_YL"
				}	
			if(wdsc=="OFF"){
				wdsc="tishi('还没开通此功能哦')"
			...
			if(ylc=="OFF"){
				ylc="tishi('还没开通此功能哦')"
				ylc_sty="style='-webkit-filter:grayscale(1)'"
			}else if(ylc=="M_YL"){
				ylc="goUrl('"+ylc+"')" 	
			}
			if(fx!==0){
		...
		html+='<div id="first-container"><div class="banner" style="padding-right:10px;overflow:hidden">'
			...
			+'<em class="tag emIco" style="width:35px;height:35px;margin:0 5px 0 16px;background-position:center -680px"></em>理财推荐<label style="position:absolute;right:10px;color:#6297F8">买理财，赚收益，还可得微信红包</label></div>'
			  for(;i<il;i++){
				 ...
				 +'</div>' 
  				...
  				createNode(document.getElementById("first-container"),"div",{html:html},"p3")
			}
		}
