...
,WARTER:function($opt){
	...
	case "KAQUAN":
		getData=function(){
			var ajax=setAjax('get',HOST.TX+"/api/resty/getRealRedPackage?countdown=100&yyyappid="+PAGE.yyyappid+"&num=20");		
				ajax.callBack=function($data){
					...
					if(data.status=='ok'){
						...
						if(len>0){
							for(var i=0;i<len;i++){
								a=toObject(arr[i]);   
								msgA.unshift({headImg:getHead(a.headimgurl),data:"刚刚摇到余额提现锦囊",category:3});
							}
						}
					}
				};
...
,JINGCAI:function($options){
	...
	,INTERACTIVE:function(sceneData,arr,type,counter_name){
		...
		return {display:function($ele,$fun,$err){		
			var menu=mainVar.mainData.menus,m=0,ml=menu.length,mi,mv,src="//a-h5.mtq.tvm.cn/yao/common/img/menu/"
			,defaultMenu={
			M_WDSC:{img:src+"n_scj.png","activeimg":1,txt:"",link:'M_WDSC',order:1},
			M_KDS:{img:src+"n_kds.png","activeimg":1,txt:"",link:'M_KDS',order:2},
			M_SC_NEW:{img:src+"n_hye.png","activeimg":1,txt:"",link:'M_SC_NEW',order:3},
			M_YL:{img:src+"n_wjb.png","activeimg":1,txt:"",link:'M_YL',order:4},
			M_GRZX:{img:src+"n_grzx.png","activeimg":1,txt:"",link:'M_GRZX',order:5}
		}
			PAGE.mascot.word=[];
				for(var _item,_dm,_img,_link;m<ml;m++){
					_item=menu[m]
					_dm=defaultMenu[_item.txt]
					if(_dm){
						_img=trim(_item.img)
						_link=trim(_item.link)
						PAGE.mascot.word.push({img:_img!=""?_img:_dm.img,order:_dm.order,link:"javascript:goUrl('"+(_link!=""?_link:_dm.link)+"')"})
					}
				}
				PAGE.mascot.word.sort(function(a,b){
